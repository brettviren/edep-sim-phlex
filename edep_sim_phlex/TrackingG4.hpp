...
// edep_sim_phlex/TrackingG4.hpp
//
// ORIGINAL self-contained implementation of the edep-sim tracking node,
// preserved as `TrackingG4`.  It owns its dedicated Geant4 thread and handshake
// directly.  The class named `Tracking` (Tracking.hpp) now provides the SAME
// interface on top of the reusable EDepSim::TrackingService (see edep-sim's
// service/); this original is kept for comparison and fallback.
//
...
// static destructors nothing to do.  ~TrackingG4 signals the thread to stop and
...
namespace edep_sim_phlex {
    ...
    class TrackingG4 {
    public:
        explicit TrackingG4(phlex::configuration const& config);
        ~TrackingG4();

        TrackingG4(TrackingG4 const&) = delete;
        TrackingG4& operator=(TrackingG4 const&) = delete;
    ...
    private:
        ...
        G4Worker* worker_ = nullptr; // heap-allocated; stopped, joined and deleted in ~TrackingG4
    };
